...
= 2.11.10 =
Security release. Two factor is now decided across the whole network, the file integrity scan no longer approves content an earlier rule hid, and the server rules no longer trust a forwarded IP header alone. Accounts left unverified by email verification are now blocked until they verify.
* Improved: on a network, the second factor settings are no longer read on every request of every site. They were resolved when the modules load, which meant switching to the main site and loading its whole autoloaded option set on each front page view of each subsite; they are now read only when a login actually needs them.
* Fix: two-factor authentication is now decided, and enforced, for the whole network instead of per site. The settings, the enforced roles, the method and the enrolment were all read from whichever site the login arrived at, while the session cookie WordPress issues is valid across the network, so somebody holding the password of an account protected by two factors could send the login to a site where that account has no role, or to one with the Login Security module switched off, never be asked for a code, and come out with a session valid everywhere. A second factor is now required if any site the account belongs to asks for it, each with its own roles and exclusions, and the check is registered on every site of a network whatever that site has switched on. Which factor is asked for is a property of the account and not of the site the login lands on: an enrolment already made is used wherever in the network it was set up, and otherwise a code by email is used if any site asking wants one, so a network that mixes the two settings no longer lets a login through for lack of an enrolment. A site whose Login Security module is off asks for nothing, and a method this version does not understand is read as the default instead of leaving the account unprotected. Single sites keep behaving as they did.
* Fix: email verification now actually blocks a login. The value written to mark an account as unverified was stored as an empty string, which the check read as "this account predates the feature, let it in", so the branch that blocks was never reached and every account passed as verified. Accounts left pending by earlier versions are now blocked as they always should have been, and resending a verification email can no longer send an already verified account back to pending.
* Fix: the file integrity scan no longer adopts as approved the very content the previous rule hid. A line carrying Vigilant's original-value marker with something else on it satisfied the one-off re-base that runs after updating, which wrote it into the approved baseline and rewrote the stored copy so the difference stopped being shown. A file is now re-based only when every marked line is wholly a comment, decided by reading the file the way PHP reads it rather than by the shape of the line, so a marked line placed before the opening tag, after a closing tag or closing a block comment counts too, and only when what the line carries would still be harmless once the marker is taken off, since restoring the value is what the marker is for. Anything else is reported, which is what the log entry already promised.
* Fix: the maintenance routines that run on an admin screen now require an administrator. They were attached to admin_init, which admin-ajax.php fires before deciding who is asking, so an anonymous request could choose the moment they run: they rewrite wp-config.php, rewrite the root .htaccess, move user meta and can rebuild the integrity baseline. The scheduled scan still completes the one-off transition on its own, so a site whose dashboard nobody opens is not left on the older rule.
* Fix: the server rules written to .htaccess no longer trust a forwarded IP header on their own. The whitelist exceptions honoured X-Forwarded-For, X-Real-IP or CF-Connecting-IP without checking where the connection came from, so anyone reaching the origin directly and sending a whitelisted address in that header skipped the Apache filters for SQL injection, XSS, traversal, methods and bad bots. The header now only counts from an address in Trusted proxy IPs, and with no list configured the exception is not written at all, so behind a CDN the Apache filters apply to every visitor until those addresses are listed. This is the other half of the finding fixed in PHP in 2.11.9.
* Fix: a request that did not come through the custom login URL no longer reveals it. Several are let through on purpose, a POST so that remote managers can authenticate and the password recovery and reset pages so they keep working, and every page rendered from them carried the secret address, because Vigilant rewrites the form action and the login links to it: a plain anonymous request for the lost password page handed it over three times. Those rewrites are now dropped for any request that did not arrive through the secret address, while authentication, password recovery and a verification in progress go on working exactly as before. Two more ways out of it are closed with them, both reachable by anyone with a single request and neither of them going through wp-login.php: wp-signup.php, which the core answers with a redirect to the registration URL, and the links of the email verification feature, which answered a made-up token with a redirect to the login page.
* Fix: on a network, the trusted device cookie of two-factor authentication is no longer sent to sites other than the one that issued it. The cookie covered the whole network while the record that validates it belongs to a single site, so its secret travelled to sites that had no business seeing it. It is now scoped to that site's own host and path, which separates the sites of a network by subdomains and of one by subdirectories alike.
* Fix: each of the two activity log checkboxes now governs its own half of the login records. Unchecking "Successful logins" also silenced failed attempts, lockouts, logins blocked by a forced reset and the probes of the hidden login and wp-admin, and with them the audit alerts, so a site under attack looked quiet. Note the other direction of the same change: those five answer to "Failed login attempts" now, so a site with that box unchecked stops recording them.
* Fix: the file integrity scan now finds wp-config.php when it lives one directory above WordPress, a layout the core supports and many hardened installations use. It was only looked for inside the WordPress directory, so on those sites the file was never compared and the module reported everything in order without having opened it.
* Fix: turning the User Security module off no longer lets back in the accounts it had already locked. A forced password reset and a registration waiting for approval are marks on an account, and both stopped being enforced with the module off, so those people logged in again with their old password while the plugin still said they could not.
* Fix: the Trusted proxy IPs list now rejects a range too wide to name a proxy, and ignores one already stored. A wildcard was rejected but the same thing written as a range was not: 0.0.0.0/0, and in two entries 0.0.0.0/1 and 128.0.0.0/1, turned every visitor into a trusted proxy. Ranges down to /8 for IPv4 and /7 for IPv6 are accepted, which covers a private network written any of the usual ways and every range the CDNs publish for themselves. Both halves also read the list the same way now, so an address written in another notation is the same proxy for the server rules and for PHP.
* Fix: on a network, an administrator of the main site without network permissions can no longer edit the Trusted proxy IPs list. It decides which connections may set the header the firewall of the whole installation acts on, so it belongs with the settings that only a network administrator can change, where its companion setting already was.
* Fix: on a network, the session limit is no longer enforced. WordPress keeps the sessions of an account for the whole network and not per site, so a limit set on one site counted, and with Close oldest session closed, the sessions that person had open on other sites, an administrator session elsewhere included. The settings are still saved and the screen now explains it, and a network-wide policy is planned. On a single site nothing changes. Reported by the wordpress.org automated security review of 2.11.9.
* Fix: on a network, the registration approval queue is now one per site, and an account held back on any site is blocked on all of them. The pending flag was a single user meta shared by the whole network, so an administrator of one site listed, approved and rejected the accounts waiting on another, and approving cleared the flag for everyone. Each site keeps its own queue now, and the accounts already waiting are moved to the sites they belong to when you update. Since the session cookie is valid across the network, the block is network-wide even though the queue is not, or an account held back on one site would log in through another and walk back in with that cookie. An account with no role yet, which is usual while it waits, now shows up in the queue instead of being blocked and invisible; rejecting one clears its mark, which on a network deleting the account does not do, so Reject is no longer a loop that leaves the row in the queue for ever; and a mark left behind by a site that has since been deleted no longer blocks an account nobody can unblock. On a single site nothing changes. Reported by the wordpress.org automated security review of 2.11.9.
* Fix: the copy of wp-config.php kept for the file integrity diff now redacts values that are not quoted text. Only strings were redacted, so a credential written as a number, such as a service token passed to define or the numeric password of an object cache, was stored as it is in the database. Numbers are redacted now wherever a value can sit, and kept only where one cannot be a credential, such as an array index or a known core setting, which is how the rest of that redaction has worked since 2.11.8. A number that is both, kept for its position and in force as a value somewhere else, is redacted in both places: left as it was, it survived the redaction, the check that runs afterwards found it and refused to store anything, and that file lost its line diff for good. Reported by the wordpress.org automated security review of 2.11.9.
