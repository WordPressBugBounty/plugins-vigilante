...
A critical finding in a scan sends the scan email with the default notification setting, and a warning follows that setting. On a network, a critical finding of a scan also reaches the network administration email, whatever the notification setting of the main site and even if the scan does not finish; when that address is also a notification recipient, the first time it gets both emails. Outside a scan, critical findings (after an update, on a version change or in the daily check), a downgrade, wherever it is detected, a version change that WordPress.org cannot confirm and a task removed again are emailed regardless of that setting, once per set of findings, to the notification recipients and, on a network, also to the network administration email unless a scan already sent it that set. Findings about the manifest or the version cannot be hidden with the ignore list, and updating to this version removes Vigilant's own files from that list. On a single site, the only way to stop these emails is to switch off self-protection under File Integrity, Scan Scope, or to remove every notification recipient.
...
1. **In WordPress:** the File Integrity tab shows a "Vigilant self-check" line with the result of the last check, and Security Check includes a "Vigilant self-protection" check.
